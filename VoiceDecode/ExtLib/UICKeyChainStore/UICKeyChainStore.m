...
+ (void)setData:(NSData *)data forKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup {
	...
	if (status == errSecSuccess) {
        if (data) {
            ...
            if (status != errSecSuccess) {
                NSLog(@"%s|SecItemUpdate: error(%ld)", __func__, (NSInteger)status);
            }
        } else {
		...
		if (status != errSecSuccess) {
			NSLog(@"%s|SecItemAdd: error(%ld)", __func__, (NSInteger)status);
		}		
	} else {
		NSLog(@"%s|SecItemCopyMatching: error(%ld)", __func__, (NSInteger)status);
	}
}
...
+ (void)removeItemForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup {
	...
	if (status != errSecSuccess && status != errSecItemNotFound) {
		NSLog(@"%s|SecItemDelete: error(%ld)", __func__, (NSInteger)status);
	}
}
